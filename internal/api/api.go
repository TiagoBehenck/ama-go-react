...
import (
	...
	"github.com/TiagoBehenck/ama-go-react.git/internal/store/pgstore"
...
func NewHandler(q *pgstore.Queries) http.Handler {
	a := apiHandler{
		...
		subscribers: make(map[uuid.UUID]map[*websocket.Conn]context.CancelFunc),
		mu:          &sync.Mutex{},
	}
...
func (h apiHandler) handleSubscribe(w http.ResponseWriter, r *http.Request) {
	...
	slog.Info("new client connected", "room_id", roomID)
	h.subscribers[roomID][c] = cancel
	h.mu.Unlock()

	<-ctx.Done()

	h.mu.Lock()
	delete(h.subscribers[roomID], c)
	h.mu.Unlock()
}
...
func (h apiHandler) handleReactToMessage(w http.ResponseWriter, r *http.Request) {
	...
	messageID, err := uuid.Parse(message_id)
	if err != nil {
		http.Error(w, "Invalid message ID", http.StatusBadRequest)
		return
	}
	...
	data, _ := json.Marshal(response{Count: reaction_count})
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	w.Write(data)
...
func (h apiHandler) handleRemoveReactFromMessage(w http.ResponseWriter, r *http.Request) {
	...
	rawMessage := chi.URLParam(r, "message_id")
	messageID, err := uuid.Parse(rawMessage)
	if err != nil {
		http.Error(w, "Invalid message ID", http.StatusBadRequest)
		return
	}
